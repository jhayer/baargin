...
def main():
    ...
    parser = argparse.ArgumentParser(description="To compile CARD RGI tabular \
        results for several samples, as table with 0 and 1 for absence/presence")
    parser.add_argument("-i", "--input_dir",
                        help="input directory containing all card tsv \
    ...
    parser.add_argument("-s", "--files_suffix",
                        help="Suffix of card ouput files to remove to get \
                        sample_ids", required=False)

    args = parser.parse_args()

    # remove suffix in files names
    suffix="_raw_RGI_main.txt"
        ...
        #    print(entry.name)
            if entry.is_file() and entry.name.endswith('.txt'):
                ...
                for line in tsv_handle:
                    lst_line = line.split('\t')
                    gene_symbol = lst_line[8]
                    # retrieving info about gene, type of AMR
                    gene_info_lst = [lst_line[14],lst_line[15],lst_line[16]]
