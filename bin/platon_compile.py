...
def main():
    ...
    parser = argparse.ArgumentParser(description="To compile Platon tsv \
        results for several samples, as table with 0 and 1 for absence/presence")
    parser.add_argument("-i", "--input_dir",
                        help="input directory containing all inc_type or plasmids \
                        or amr_hits in tsv from json2tsv_platon", required=True)
    parser.add_argument("-o", "--output_file",
                        help="output tabular file containing detected inc types \
                        or plasmids_hits or amr_hits for all samples", required=True)
    parser.add_argument("-t", "--type",
                        help="type of tsv input: inc, plasmid or amr", \
                        required=True)
    ...
    suffix="_platon_inctypes_raw.tsv"
